...
SnakeGame::SnakeGame(sf::Vector2u windowSize) :
	...
	snake_{ vector2Cast<float>((getWindowSize().x / 2.f) - (getWindowSize().x / 2) % 16, getWindowSize().y / 2.f - (getWindowSize().y / 2) % 16), snakeStartDirection_, entityWidth_, snakeMovementPerFrame_, snakeColor_ }, 
...
void SnakeGame::update(const sf::Time& deltaTime)
 {
	...
	const sf::Time timeStep{ sf::seconds(10.f / framesPerSecond_) };
